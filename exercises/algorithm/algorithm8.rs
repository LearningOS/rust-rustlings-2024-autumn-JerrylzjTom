...
impl<T> myStack<T> {
    ...
    pub fn push(&mut self, elem: T) {
        if self.q1.is_empty() {
            self.q2.enqueue(elem); // Push to q2 if q1 is empty
        } else {
            self.q1.enqueue(elem); // Push to q1 otherwise
        }
    }
    pub fn pop(&mut self) -> Result<T, &str> {
        ...
        if !self.q1.is_empty() {
            ...
            // Pop the last element from q1 (which simulates stack pop)
            self.q1.dequeue()
        } else {
            while self.q2.size() > 1 {
                // Move elements from q2 to q1 except the last one
